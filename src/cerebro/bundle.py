...
class BundleValidator:
    def validate(self, bundle: SemanticBundle) -> ValidationReport:
            ...
            for link in obj.links:
                if link not in by_id:
                    issues.append(
                        ValidationIssue(
                            code="dangling_link",
                            message=f"{obj.id} links to missing {link}",
                            path=obj.path,
                        )
                    )
        ...
        return ValidationReport(
            valid=not issues, document_count=len(bundle.objects), issues=issues
        )
        ...
        if cardinality not in ALLOWED_CARDINALITIES:
            issues.append(
                ValidationIssue(
                    ...
                    message=f"{obj.id} has invalid cardinality {cardinality}",
                    path=obj.path,
                )
            )
    ...
    def _validate_metric(
        ...
        for dependency in dependencies:
            if dependency not in by_id:
                issues.append(
                    ValidationIssue(
                        code="unresolved_metric_dependency",
                        message=f"{obj.id} depends on missing {dependency}",
                        path=obj.path,
                    )
                )
        ...
        if not obj.cerebro.get("formula"):
            issues.append(
                ValidationIssue(
                    ...
                    message=f"{obj.id} has no formula",
                    path=obj.path,
                )
            )
