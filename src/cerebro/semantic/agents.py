...
class SemanticInventoryAgent:
    ...
    def run(self, snapshot: CatalogSnapshot) -> BusinessSemantics:
        ...
        prompt = (
            ...
            "and dimension binding must use real table and column names. "
            "Also propose a small set of business domains (typically 2-6) that group the proposed "
            "entities into coherent business areas, each with an id, name, description, "
            "classification, and an optional owner. Assign each entity's domain field to the id of "
            "the one proposed domain it most clearly belongs to; leave an entity's domain unset "
            "rather than forcing it into a domain that does not fit. Do not propose a domain with "
            "no entities assigned to it. "
            "Policies may be proposed "
