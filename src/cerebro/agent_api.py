...
def create_agent_router(runtime: AgentRuntime) -> APIRouter:
    ...
    # A plain `def` on purpose. DuckDB validation and execution block, and the
    # executor owns a lock, so Starlette running this in its threadpool is both
    # correct and keeps one slow question from stalling the event loop.
    @router.post("/ask")
    def ask(payload: AgentAskRequest) -> JSONResponse:
        try:
