...
`DuckDB catalog -> sanitized CatalogSnapshot -> one orchestrator / three typed model calls -> deterministic compiler + validator -> whole-candidate review -> immutable reviewed bundle -> explicit activation`
...
- FR-901: Generation accepts `configured` and `database_only`; omission remains `configured` for API compatibility and the Build UI requires an explicit choice.
...
- FR-906: The one generation orchestrator makes bounded typed calls for business, relationship, and query semantics. Model inputs contain only sanitized catalog facts; database facts remain `discovered` and generated semantics remain `ai_proposed`.
...
The initial implementation is local and single-user. Reviewer identity is self-declared. Per-object editing, authentication, signatures, uploads, non-DuckDB sources, concurrent agent swarms, row sampling, and web enrichment are out of scope.
